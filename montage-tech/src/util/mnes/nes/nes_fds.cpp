...
extern uint32 LinearToLog(int32 l);

extern int32 LogToLinear(uint32 l, uint32 sft);

extern uint32 DivFix(uint32 p1, uint32 p2, uint32 fix);

#ifndef NES_BASECYCLES
		...
		switch (address & 15)
		{
			...
			case 4:
				...
				else if(apu->fdssound.op[1].sweep)
				{
					...
					pop->volume=abs((int)(apu->fdssound.op[1].sweep - apu->fdssound.op[1].spd));
...
static void FDSSoundReset(void)
{
  ...
  NES_MEMSET(&apu->fdssound, 0, sizeof(FDSSOUND));
