...
#endif
			{
				if(apu->ym2413s.ch[ch].toneno != (value >> 4))
				{
					if (value >> 4)
					{
						...
						NES_MEMCPY(apu->ym2413s.ch[ch].tone, oplltone[value >> 4], 8);
					...
					else if ((apu->ym2413s.ch[ch].toneno != 0x10) || (apu->ym2413s.toneupdate & (1 << ch)))
					{
						...
						NES_MEMCPY(apu->ym2413s.ch[ch].tone, apu->ym2413s.usertone, 8);
						apu->ym2413s.ch[ch].update |= OPLL_UPDATE_TONE;
					}
				}
				apu->ym2413s.ch[ch].op[1].tl = (value & 0xF) << 2;
...
static void OPLLSoundReset(void)
{
	...
	double a;
	uint32 cps;
	NES_MEMSET(&apu->ym2413s, 0, sizeof(OPLLSOUND));
	...
	for (i = 1; i < w; i++)
	{
		...
		ua = (uint32)((LOG_LIN_BITS - (log(a) / log((double)2))) * (1 << LOG_BITS));
