...
SNSS_ReadBlockHeader (SnssBlockHeader *header, SNSS_FILE *snssFile)
{
  ...
  if (vfs_read (headerBytes, 12, 1, snssFile->fp) != 12)
...
SNSS_WriteBlockHeader (SnssBlockHeader *header, SNSS_FILE *snssFile)
{
  ...
  NES_MEMCPY (headerBytes, header->tag, TAG_LENGTH);
  tmp_uint32 = htonl (header->blockVersion);
  NES_MEMCPY(headerBytes + 4, &tmp_uint32, 4);
  tmp_uint32 = htonl (header->blockLength);
  NES_MEMCPY(headerBytes + 8, &tmp_uint32, 4);
  
  if (vfs_write (headerBytes, 12, 1, snssFile->fp) != 1)
...
SNSS_ReadFileHeader (SNSS_FILE *snssFile)
{
  ...
  if (vfs_read (buf, 8, 1, snssFile->fp) != 8)
    return SNSS_READ_FAILED;
...
SNSS_WriteFileHeader (SNSS_FILE *snssFile)
{
  ...
  NES_MEMCPY(writeBuffer + 4, &tmp_uint32, 4);
  
  if (vfs_write (writeBuffer, 8, 1, snssFile->fp) != 1)
    return SNSS_WRITE_FAILED;
...
/**************************************************************************/
SNSS_RETURN_CODE
SNSS_OpenFile (SNSS_FILE **snssFile, const uint16 *filename, SNSS_OPEN_MODE mode)
{
  SNSS_RETURN_CODE error_code, return_code;
  
  *snssFile = NES_MALLOC(sizeof(SNSS_FILE));
  if (NULL == *snssFile)
  {
  ...
  if (SNSS_OPEN_READ == mode)
    (*snssFile)->fp = vfs_open (filename, VFS_READ);
  else 
  {
    (*snssFile)->fp = vfs_open (filename, VFS_WRITE);
...
SNSS_ReadBaseBlock (SNSS_FILE *snssFile)
{
  ...
  if (vfs_read (blockBytes, BASE_BLOCK_LENGTH, 1, snssFile->fp) != BASE_BLOCK_LENGTH) 
    return SNSS_READ_FAILED;
  ...
  NES_MEMCPY (snssFile->baseBlock.cpuRam, blockBytes + 0x9, 0x800);
  NES_MEMCPY (snssFile->baseBlock.spriteRam, blockBytes + 0x809, 0x100);
  NES_MEMCPY (snssFile->baseBlock.ppuRam, blockBytes + 0x909, 0x1000);
  NES_MEMCPY (snssFile->baseBlock.palette, blockBytes + 0x1909, 0x20);
  NES_MEMCPY (snssFile->baseBlock.mirrorState, blockBytes + 0x1929, 0x4);
...
SNSS_WriteBaseBlock (SNSS_FILE *snssFile)
{
  ...
  NES_MEMCPY (blockBytes + 0x5, &tmp_uint16, 2);
  blockBytes[0x7] = snssFile->baseBlock.reg2000;
  blockBytes[0x8] = snssFile->baseBlock.reg2001;
  NES_MEMCPY (blockBytes + 0x9, snssFile->baseBlock.cpuRam, 0x800);
  NES_MEMCPY (blockBytes + 0x809, snssFile->baseBlock.spriteRam, 0x100);
  NES_MEMCPY (blockBytes + 0x909, snssFile->baseBlock.ppuRam, 0x1000);
  NES_MEMCPY (blockBytes + 0x1909, snssFile->baseBlock.palette, 0x20);
  NES_MEMCPY (blockBytes + 0x1929, snssFile->baseBlock.mirrorState, 0x4);
  tmp_uint16 = htons (snssFile->baseBlock.vramAddress);
  NES_MEMCPY (blockBytes + 0x192D, &tmp_uint16, 2);
  blockBytes[0x192F] = snssFile->baseBlock.spriteRamAddress;
  blockBytes[0x1930] = snssFile->baseBlock.tileXOffset;
  
  if (vfs_write (blockBytes, BASE_BLOCK_LENGTH, 1, snssFile->fp) != 1)
    return SNSS_WRITE_FAILED;
...
SNSS_WriteVramBlock (SNSS_FILE *snssFile)
{
  ...
  if (vfs_write (snssFile->vramBlock.vram, 
...
SNSS_ReadSramBlock (SNSS_FILE *snssFile)
{
  ...
  if (vfs_read (&snssFile->sramBlock.sramEnabled, 1, 1, snssFile->fp) != 1)
    return SNSS_READ_FAILED;
  
  /* SRAM size is the size of the block - 1 (SRAM enabled byte) */
  if (vfs_read (&(snssFile->sramBlock.sram),
             header.blockLength - 1, 1, snssFile->fp) != header.blockLength - 1)
    return SNSS_READ_FAILED;
...
SNSS_WriteSramBlock (SNSS_FILE *snssFile)
{
  ...
  if (vfs_write (&(snssFile->sramBlock.sramEnabled), 
              1, 1, snssFile->fp) != 1) 
    return SNSS_WRITE_FAILED;
  
  if (vfs_write (snssFile->sramBlock.sram, 
...
SNSS_ReadMapperBlock (SNSS_FILE *snssFile)
{
  ...
  if (vfs_read (blockBytes, header.blockLength, 1, snssFile->fp) != header.blockLength)
    return SNSS_READ_FAILED;
  ...
  NES_MEMCPY (&snssFile->mapperBlock.extraData.mapperData, &blockBytes[0x18], 0x80);
...
SNSS_WriteMapperBlock (SNSS_FILE *snssFile)
{
  ...
  for (i = 0; i < 4; i++) 
  {
    ...
    NES_MEMCPY (blockBytes + i * 2, &tmp_uint16, 2);
  }
  
  for (i = 0; i < 8; i++) 
  {
    tmp_uint16 = htons (snssFile->mapperBlock.chrPages[i]);
    NES_MEMCPY (blockBytes + 0x8 + i * 2, &tmp_uint16, 2);
  }
  
  NES_MEMCPY (&blockBytes[0x18], &snssFile->mapperBlock.extraData.mapperData, 0x80);
  
  if (vfs_write (blockBytes, MAPPER_BLOCK_LENGTH, 1, snssFile->fp) != 1)
    return SNSS_WRITE_FAILED;
...
SNSS_ReadSoundBlock (SNSS_FILE *snssFile)
{
  ...
  if (vfs_read (snssFile->soundBlock.soundRegisters, 
             header.blockLength, 1, snssFile->fp) != header.blockLength)
    return SNSS_READ_FAILED;
...
SNSS_WriteSoundBlock (SNSS_FILE *snssFile)
{
  ...
  if (vfs_write (snssFile->soundBlock.soundRegisters, 
