...
#include "gamenum.h"
typedef GameNum<16, uint32> fixed16;
...
#include "nes_mapper_friend_decl.h"
  
public:
  NES(sound_mgr* _sound_mgr);
  bool initialize (const uint16 *filename);
  bool initialize_rom_on_ram(void *p_rom);
  ~NES();
...
/*   float CYCLES_PER_LINE;				// *FP* */
/*   float CYCLES_BEFORE_NMI;				// *FP* */
  fixed16 CYCLES_PER_LINE;
  fixed16 CYCLES_BEFORE_NMI;
  ...
  uint8 DiskAccessed();
  uint16 disksys_rom_filename[256];
  ...
  /* NNNesterJ cheat code */
  void load_nnnesterj_cheat (const uint16 *filename);
...
/*   float  ideal_cycle_count;   // number of cycles that should have executed so far *FP* */
  fixed16  ideal_cycle_count;   // number of cycles that should have executed so far
  ...
  bool loadROM(const uint16* fn); 
  bool loadROM_on_ram(void *p_rom); 
  void freeROM();
...
/*   void  emulate_CPU_cycles(float num_cycles); /\* *FP* *\/ */
  void  emulate_CPU_cycles(fixed16 num_cycles);
