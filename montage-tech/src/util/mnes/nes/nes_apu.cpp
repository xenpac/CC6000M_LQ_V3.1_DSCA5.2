...
** #included stlib.h for NES_MALLOC()
**
** - Darren Ranalli
*/
...
void apu_process(void *buffer, int num_samples)
{
  ...
  int16 *p_buf_16 = (int16 *)buffer;
  int8 *p_buf_8 = (int8 *)buffer;
...
/* Overflow fixed by T.Yano Dec.12.2000 */
      ...
      {
        *p_buf_16++ = (int16)accum;
      }
      else
      {
        *p_buf_8++ = (accum >> 8) ^ 0x80;
      }
    }
  }
...
void apu_reset_apus(APUSOUND *apus)
{
   ...
   // Reset rectangles
   for ( i=0; i<2; i++) {
      NES_MEMSET(&apus->rectangle[i], 0, sizeof(apus->rectangle[i]));
   }
...
#endif
   NES_MEMSET(&apus->triangle, 0, sizeof(apus->triangle));
#ifdef APU_YANO
   apus->triangle.ideal_triangle = mode;
#endif
   NES_MEMSET(&apus->noise, 0, sizeof(apus->noise));
   NES_MEMSET(&apus->dmc, 0, sizeof(apus->dmc));
}
...
void apu_reset(void)
{
   ...
   NES_MEMSET(&apu->queue, 0, APUQUEUE_SIZE * sizeof(apudata_t));
   apu->q_head = apu->q_tail = 0;

   NES_MEMSET(&apu->ex_queue, 0, APUQUEUE_SIZE * sizeof(apudata_t));
...
apu_t *apu_create(int sample_rate, int refresh_rate, int frag_size, int sample_bits)
{
  ...
  temp_apu = (apu_t *)NES_MALLOC(sizeof(apu_t));
  if (NULL == temp_apu)
    return NULL;
