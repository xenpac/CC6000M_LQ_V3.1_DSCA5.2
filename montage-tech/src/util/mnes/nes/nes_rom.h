...
public:
  ...
  bool initialize_on_ram (void *p_rom);
  ...
  const uint16* GetRomName() { return rom_name; }
  const uint16* GetRomPath() { return rom_path; }
...
private:
  ...
  bool load_rom (const uint16 *filename);
  bool load_rom_on_ram (void *p_rom);
  rom_image_type load_nes_rom (hfile_t fp, bool isZip);
  rom_image_type load_fds_rom (hfile_t fp, bool isZip);
  rom_image_type load_nes_rom_on_ram (void *p_rom, bool isZip);
  rom_image_type load_fds_rom_on_ram (void *p_rom, bool isZip);
  void correct_rom_image ();
  uint32 calc_crc();
  void set_path_info(const uint16 * fn);
