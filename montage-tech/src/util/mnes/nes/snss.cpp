...
NES_SNSS::adopt_BASR(SnssBaseBlock* block, NES* nes)
{
  ...
  // RAM
  NES_MEMCPY(nes->RAM, block->cpuRam, 0x800);

  // SPR-RAM
  NES_MEMCPY(nes->ppu->spr_ram, block->spriteRam, 0x100);

  // PPU $2000-$2FFF (Name Tables/Attrib Tables)
  NES_MEMCPY(nes->ppu->PPU_nametables, block->ppuRam, 4*0x400);

  // palettes
  NES_MEMCPY(nes->ppu->bg_pal,  &block->palette[0x00], 0x10);
  NES_MEMCPY(nes->ppu->spr_pal, &block->palette[0x10], 0x10);
...
NES_SNSS::adopt_VRAM(SnssVramBlock* block, NES* nes)
{
  ...
  // read MAX 32KB
  NES_MEMCPY(nes->ppu->get_patt(), &block->vram, block->vramSize);
...
NES_SNSS::adopt_SRAM(SnssSramBlock* block, NES* nes)
{
  ...
  // read SRAM
  context = nes6502_get_current_context ();
  NES_MEMCPY(context->mem_page[3], block->sram, (block->sramSize <= 0x2000) ? block->sramSize : 0x2000);
  nes6502_setcontext();
    ...
    case 1:
      {
        ...
        NES_MEMCPY(mapper->regs, mapper_data->registers, 4);
...
analyze_blockheader(SnssBlockHeader *sbh, uint8 *header)
{
  ...
  NES_MEMSET(sbh, 0, sizeof(SnssBlockHeader));
  NES_MEMCPY(sbh->tag, header, 4);
  NES_MEMCPY(&v, header + 4, 4);
  NES_MEMCPY(&s, header + 8, 4);
...
NES_SNSS::adopt_ExMPRD(SNSS_FILE *snssFile, NES* nes)
{
  ...
  if (mapper_num == 1 && nes->crc32() == 0xb8747abf)
  {
    ...
    // save WRAM
    vfs_seek(snssFile->fp, 12, VFS_SEEK_CUR);
    vfs_read (nes->SaveRAM + 0x2000, 0x2000, 1, snssFile->fp);
  }
  else if (mapper_num == 5)
  {
    ...
    // read WRAM
    vfs_seek(snssFile->fp, 12, VFS_SEEK_CUR);
    vfs_read (mapper->wram, 0x10000, 1, snssFile->fp);
    NES_MEMCPY (nes->SaveRAM, mapper->wram, 0x10000);
  }
  else if (mapper_num == 20)
  {
    ...
    vfs_read(header, 1, 12, snssFile->fp);
    if (NES_MEMCMP(header, "WRAM", 4)) 
      goto error;
    vfs_read(mapper->wram, 1, 0x8000, snssFile->fp);
    
    vfs_read(header, 1, 12, snssFile->fp);
    if (NES_MEMCMP(header, "FDSD", 4))
      goto error;
    if (vfs_read(buf, sizeof(buf), 1, snssFile->fp) != sizeof(buf))
      goto error;
...
boolean
NES_SNSS::LoadSNSS(const uint16* fn, NES* nes)
{
...
NES_SNSS::extract_BASR(SnssBaseBlock* block, NES* nes)
{
  ...
  // RAM
  NES_MEMCPY(block->cpuRam, nes->RAM, 0x800);

  // SPR-RAM
  NES_MEMCPY(block->spriteRam, nes->ppu->spr_ram, 0x100);

  // PPU $2000-$2FFF (Name Tables/Attrib Tables)
  NES_MEMCPY(block->ppuRam, nes->ppu->PPU_nametables, 4*0x400);

  // palettes
  NES_MEMCPY(block->palette + 0x00, nes->ppu->bg_pal,  0x10);
  NES_MEMCPY(block->palette + 0x10, nes->ppu->spr_pal, 0x10);
...
NES_SNSS::extract_VRAM(SnssVramBlock* block, NES* nes)
{
  ...
  NES_MEMCPY(&block->vram, nes->ppu->get_patt(), block->vramSize);
...
NES_SNSS::extract_SRAM(SnssSramBlock* block, NES* nes)
{
  ...
  // SRAM data
  context = nes6502_get_current_context ();
  NES_MEMCPY(block->sram, context->mem_page[3], 0x2000);
    ...
    case 1:
      {
        ...
        // last values written to the 4 registers
        NES_MEMCPY(mapper_data->registers, mapper->regs, 4);
		 ...
		 const char *signature, uint32 version, uint32 size)
{
  ...
  NES_MEMSET(header, 0, 12);
  NES_MEMCPY(header, signature, 4);
  NES_MEMCPY(header + 4, &v, 4);
  NES_MEMCPY(header + 8, &s, 4);
}
...
NES_SNSS::extract_ExMPRD(SNSS_FILE *snssFile, NES* nes)
{
  ...
  if(mapper_num == 1 && nes->crc32() == 0xb8747abf)
  {
    ...
    vfs_write(header, 1, 12, snssFile->fp);
    vfs_write(nes->SaveRAM + 0x2000, 1, 0x2000, snssFile->fp);
  }
  else if(mapper_num == 5)
  {
    NES_mapper5* mapper = (NES_mapper5*)nes->mapper;
    ...
    vfs_write(header, 1, 12, snssFile->fp);
    vfs_write(mapper->wram, 1, 0x10000, snssFile->fp);
  }
  else if(mapper_num == 20)
  {
    NES_mapper20* mapper = (NES_mapper20*)nes->mapper;
    ...
    vfs_write(header, 1, 12, snssFile->fp);
    vfs_write(mapper->wram, 1, 0x8000, snssFile->fp); 
    
    nes->make_savedata (buf);
    
    make_blockheader(header, "FDSD", 1, 65500);
    vfs_write(header, 1, 12, snssFile->fp);
    vfs_write(buf, 1, nes->fds_save_buflen, snssFile->fp);
  }
}
...
boolean 
NES_SNSS::SaveSNSS(const uint16* fn, NES* nes)
{
  SNSS_FILE* snssFile = NULL;
