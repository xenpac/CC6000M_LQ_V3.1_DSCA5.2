...
//   ppu->do_scanline_and_dont_draw_bankswitch_per_tile (CYCLES_PER_LINE * 32 / 42); 
  	...
  	ppu->do_scanline_and_dont_draw_bankswitch_per_tile (CYCLES_PER_LINE * 32 / 42); 
  #else
  	fixed16 t = CYCLES_PER_LINE;
  	t *= 32;
  	t /= 42;
  	ppu->do_scanline_and_dont_draw_bankswitch_per_tile ( t ); 
  #endif
  
  /* do half line's worth of CPU cycles (hblank) */ 
  emulate_CPU_cycles((fixed16)((uint32)13)); 
  mapper->HSync(i); 
  
  #if 0
  	emulate_CPU_cycles(CYCLES_PER_LINE * 10 / 42 - 13); 
  #else
  	fixed16 tt = CYCLES_PER_LINE;
  	tt *= 10;
  	tt /= 42;
  	tt -= 13;
  	emulate_CPU_cycles( tt ); 
  #endif
...
NES::emulate_frame_line_bankswitch_per_tile (int i, uint16 *cur_line)
{
  /* render line */
    ...
    t *= 32;
    t /= 42;
  	ppu->do_scanline_and_draw_bankswitch_per_tile (cur_line, t); 
  #endif
  /* do half line's worth of CPU cycles (hblank) */ 
  emulate_CPU_cycles(static_cast<fixed16>((uint32)13)); 
  mapper->HSync(i); 
  #if 0
  	emulate_CPU_cycles(CYCLES_PER_LINE * 10 / 42 - 13); 
  #else 
