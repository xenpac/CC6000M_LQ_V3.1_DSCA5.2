...
bool8_32 S9xGraphicsInit ()
{
    ...
    if (Settings.SixteenBit)
    {
	if (!(GFX.X2 = (uint16 *) NES_MALLOC (sizeof (uint16) * 0x10000)))
	    return (FALSE);

	if (!(GFX.ZERO_OR_X2 = (uint16 *) NES_MALLOC (sizeof (uint16) * 0x10000)) ||
	    !(GFX.ZERO = (uint16 *) NES_MALLOC (sizeof (uint16) * 0x10000)))
	{
	    if (GFX.ZERO_OR_X2)
	    {
		NES_FREE ((char *) GFX.ZERO_OR_X2);
		GFX.ZERO_OR_X2 = NULL;
	    }
	    if (GFX.X2)
	    {
		NES_FREE ((char *) GFX.X2);
		GFX.X2 = NULL;
	    }
	    return (FALSE);
	}
...
void S9xDisplayStringEx (const char *string, uint8 *screen, uint32 pitch, uint8 x, uint8 y)
{
    uint8 *Screen = screen + 2 + x + y * pitch;
    int len = strlen (string);
    int max_chars = /*IPPU.RenderedScreenWidth*/320 / (font_width - 1);
    int char_count = 0;
    int i;
...
void S9xDisplayString16 (const char *string, uint16 *screen, uint32 pitch, uint16 x, uint16 y)
{
	...
	for (i = 0; i < len; i++, char_count++)
	{
		...
		Screen += (font_width - 1) * sizeof (uint8); 
	}
}

#endif	// _USE_SNES_FONT_
...
//	struct SPPU *ppu = &PPU;
		...
		!((gfx->r2130 & 0x30) == 0x10 && ippu->Clip[1].Count[5] == 0))
	{
	    ...
	    if (ippu->Clip [0].Count [5])
		{
			if (strncmp (Memory.ROMId, "AQT", 3) != 0)
			{
					...
					for (uint32 y = starty; y <= endy; y++)
					{
						for (uint32 w = 0; w < ippu->Clip [1].Count [5]; w++)
						{
							if (ippu->Clip [1].Right [w][5] >= ippu->Clip [1].Left [w][5])
							{
								...
								memmove (gfx->Screen + y * gfx->Pitch2 + offset,//des
									gfx->SubScreen + y * gfx->Pitch2 + offset,//src
									width);
								fprintf(stderr, "%s\n", "QWERTTY");
							}
						}
				...
				else
				{
					for (uint32 y = starty; y <= endy; y++)
					{
						for (int x = 0; x < ippu->RenderedScreenWidth * sizeof (uint16); x++)
						{
							*(gfx->Screen + y * gfx->Pitch2 + x) = *(gfx->SubScreen + y * gfx->Pitch2 + (ippu->RenderedScreenWidth * sizeof (uint16) - x));
						}
		...
		//256*224
		//fprintf(stderr, "ippu->RenderedScreenWidth = %d\n", ippu->RenderedScreenWidth);
		//fprintf(stderr, "ippu->RenderedScreenHeight = %d\n", ippu->RenderedScreenHeight);
		//for (uint32 y = 0; y < 240 / 2; y++)
		//{
		//	for (int x = 0; x < ippu->RenderedScreenWidth * sizeof (uint16); x++)
		//	{
		//		*(gfx->Screen + y * gfx->Pitch2 + x) = *(gfx->Screen + (y+120) * gfx->Pitch2 + x);
		//	}
		//}
