...
char *CMemory::Safe (const char *s)
{
	...
	if (safe)
	    NES_FREE ((char *) safe);
	safe = (char *) NES_MALLOC (safe_len = len + 1);
    }
...
{
    RAM	    = (uint8 *) NES_MALLOC (0x20000);
    SRAM    = (uint8 *) NES_MALLOC (0x20000);
    VRAM    = (uint8 *) NES_MALLOC (0x10000);
    ROM     = (uint8 *) NES_MALLOC (MAX_ROM_SIZE + 0x200 + 0x8000);
    FillRAM = NULL;

    IPPU.TileCache [TILE_2BIT] = (uint8 *) NES_MALLOC (MAX_2BIT_TILES * 128);
    IPPU.TileCache [TILE_4BIT] = (uint8 *) NES_MALLOC (MAX_4BIT_TILES * 128);
    IPPU.TileCache [TILE_8BIT] = (uint8 *) NES_MALLOC (MAX_8BIT_TILES * 128);
    
    IPPU.TileCached [TILE_2BIT] = (uint8 *) NES_MALLOC (MAX_2BIT_TILES);
    IPPU.TileCached [TILE_4BIT] = (uint8 *) NES_MALLOC (MAX_4BIT_TILES);
    IPPU.TileCached [TILE_8BIT] = (uint8 *) NES_MALLOC (MAX_8BIT_TILES);
...
void CMemory::FreeSDD1Data ()
{
    if (SDD1Index)
    {
	NES_FREE ((char *) SDD1Index);
	SDD1Index = NULL;
    }
    if (SDD1Data)
    {
	NES_FREE ((char *) SDD1Data);
	SDD1Data = NULL;
    }
}
...
*/
	...
	uint8 *tmp = (uint8 *) NES_MALLOC (0x8000);
	if (tmp)
	{
	    for (i = 0; i < nblocks * 2; i++)
	    {
...
bool8_32 CMemory::SaveSRAM (const char *filename)
{
    ...
    // need NOT save, if SRam is Empty, OopsWare.
    int rs = size >> 2;
    int *pp = (int *) ::SRAM;
    bool IsAll0 = true;
    while (rs--) {
