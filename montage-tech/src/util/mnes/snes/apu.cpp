...
bool8_32 S9xInitAPU ()
{
    IAPU.RAM = (uint8 *) NES_MALLOC (0x10000);
    IAPU.ShadowRAM = (uint8 *) NES_MALLOC (0x10000);
    IAPU.CachedSamples = (uint8 *) NES_MALLOC (0x40000);
...
void S9xDeinitAPU ()
{
    if (IAPU.RAM)
    {
	NES_FREE ((char *) IAPU.RAM);
	IAPU.RAM = NULL;
    }
    if (IAPU.ShadowRAM)
    {
	NES_FREE ((char *) IAPU.ShadowRAM);
	IAPU.ShadowRAM = NULL;
    }
    if (IAPU.CachedSamples)
    {
	NES_FREE ((char *) IAPU.CachedSamples);
