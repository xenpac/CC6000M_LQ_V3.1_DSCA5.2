...
void S9xSetMasterVolume (short volume_left, short volume_right)
{
    if (Settings.DisableMasterVolume)
    {
		...
		SoundData.master_volume [0] = SoundData.master_volume [1] = 256;
    }
    else
    {
		if (!so.stereo)
...
void S9xMixSamplesO (uint8 *buffer, int sample_count, int byte_offset)
{
...
#endif
    {
		...
		{
			//sd->master_volume[0] = 10;
			//sd->master_volume[1] = 10;
			//fprintf(stderr, "master_volume echo_volume %d %d\n", sd->master_volume[J & 1], sd->echo_volume[J & 1]);
			//84,36,-53

			int O = byte_offset >> 1;
			if (sd->echo_enable && sd->echo_buffer_size)
			{
				if (so.stereo)
				{
					...
					if (sd->no_filter)
					{
						...
						{
							int E = Echo [sd->echo_ptr];
							
							Echo [sd->echo_ptr] = (E * sd->echo_feedback) / 128 +
								EchoBuffer [J];
							
							if ((sd->echo_ptr += 1) >= sd->echo_buffer_size)
								sd->echo_ptr = 0;
							
//							I = (MixBuffer [J] * 
//								sd->master_volume [J & 1] +
//								E * sd->echo_volume [J & 1]) / VOL_DIV16;

							I = (MixBuffer [J] * 
								m_nVolume +
								E * m_nVolume) / VOL_DIV16;
...
//								E * sd->echo_volume [0]) / VOL_DIV16;
			...
			else // if (sd->echo_enable && sd->echo_buffer_size)
			{
    ...
    else
    {
		...
		if (so.mute_sound)
		{
			...
			if (so.encoded)
			{
				for (J = 0; J < sample_count; J++)
				{
					I = (MixBuffer [J] * sd->master_volume_left) / VOL_DIV16;
					CLIP16(I);
