...
static UINT4 custom_s11_value = 0xd76aa478;
                         ...
                         const unsigned char block[64])
{
  ...
  FF (a, b, c, d, x[ 0], S11, custom_s11_value); /* 1 */
...
void md5_set_specific_value(UINT4 value)
{
   custom_s11_value = value;
}
                                   ...
                                   const unsigned char *input, unsigned int inputLen)
{
   ...
   unsigned char i = 0;
   md5_get_date_value(md5_value,input,inputLen);
   for(i = 0;i < 16;i ++)
    {
      if(md5_value[i] != check_md5_value[i])
        {
           break;
        }
    }
