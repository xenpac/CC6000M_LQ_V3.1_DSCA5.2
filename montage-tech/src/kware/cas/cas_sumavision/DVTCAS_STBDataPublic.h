...
#define DVTCA_MAXLEN_PINCODE					8		//最长的PIN码长度
...
#define	DVTCA_MAXLEN_EMAIL_SENDERNAME			11		//E-mail发送者姓名，最多10个字符+\0
#define	DVTCA_MAXLEN_EMAIL_CONTENT				1001	//E-mail最大长度，最多1000个字符+\0
...
typedef struct _SDVTCAEmailMsg{
	DWORD		m_dwVersion;										//发送时间
	char		m_szSenderName[DVTCA_MAXLEN_EMAIL_SENDERNAME];	//发送者姓名
	WORD		m_wEmailLength;									//邮件内容长度
	char		m_szEmail[DVTCA_MAXLEN_EMAIL_CONTENT];			//邮件内容
...
typedef struct _SDVTCACharaterFingerMsg{
...
}SDVTCACharaterFingerMsg;

typedef struct _SDVTCADotMatrixFingerMsg {
	bool    m_bVisionType;			//可视类型（0-Covert，1-Overt）.covert时，duration不生效。
