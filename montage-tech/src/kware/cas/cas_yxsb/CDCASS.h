...
/*------------ OSD的长度限制 -----------*/
#define CDCA_MAXLEN_OSD           360  /* OSD内容的最大长度 */
/*------------ OSD显示类型 ------------*/
...
/*OSD 结构*/
typedef	struct{
    CDCA_U8 byReserved1;	//0x0	
    CDCA_U8 byFontSize;		//0：默认，1：大号，2：小号
    CDCA_U8 byReserved2;	//	0x0	
    CDCA_U8   byBackgroundArea;		//表示占屏幕正中央部分的面积百分比，默认为80，范围（20~80）
    CDCA_U32 dwFontColor;	//支持256色
    CDCA_U32 dwBackgroundColor;		//支持256色    
    char 	    szContent[CDCA_MAXLEN_OSD+1]; //osd字符串，用0结尾的字符串
}SCDCAOSDInfo;
...
/*-- 指纹显示信息--*/
typedef struct {
CDCA_BOOL	bIsVisible; 	//0：显性显示；1：隐形显示
CDCA_U8		byFontSize; 	//0：默认值；1：大字体；2：小字体
CDCA_U8		byType; 	//0：卡号；1：机顶盒号
CDCA_U8		byReserved;	//0x00	保留
CDCA_U32	dwFontColor;	//标准RGB值
CDCA_U32	dwBackgroundColor;	//标准RGB值
CDCA_U8		byX_position;		//指纹显示位置的X轴坐标，取值范围0~100
CDCA_U8		byY_position;		//指纹显示位置的Y轴坐标，取值范围0~100
CDCA_U8		pbyReserved[7];	//0x00	保留
char*		szContent; //需要显示的指纹内容，用0结尾的字符串
}SCDCAFingerInfo;
