...
const UINT8 MxLWareDrvVersion[] = {2, 1, 16, 1};
...
MXL_STATUS MxL_ConfigDemodSymbolRate(PMXL_SYMBOL_RATE_T SymbolRatePtr) 
{
  ...
  REAL32 SPSin = 0.0, SymbolRate[SYM_NUM_B_BANK];
  UINT32 ResampleRateRatio;
...
static MXL_STATUS MxL_ConfigTunerChanDependent(PMXL_CHAN_DEPENDENT_T ParamPtr)
{
  ...
  if (ParamPtr->ChanDependentCfg == MXL_ENABLE)
  {
    ...
    {
      status |= Ctrl_WriteRegister(ParamPtr->I2cSlaveAddr, RF_SPARE_REG, 0x00F4);
    }
    else
    {
      status |= Ctrl_WriteRegister(ParamPtr->I2cSlaveAddr, RF_SPARE_REG, 0);
    }
  }
  else
  {
...
MXL_STATUS MxL_GetDemodBer(PMXL_DEMOD_BER_INFO_T BerInfoPtr) 
{
  ...
  if (AnnexType == ANNEX_A_TYPE) 
  {
    ...
    else if (QamType == QAM16)
    {
      kcer = 65788;
      kber = 394;    
      kuber = 40;
    }
    else if (QamType == QAM32)
    {
      kcer = 52630;
      kber = 315;    
      kuber = 32;
    }
    else if (QamType == QAM128)
    {
      kcer = 37593;
      kber = 225;    
      kuber = 23;
    }
    else // QAM256 by default
    {
      kcer = 32894;
      kber = 197;
      kuber = 20;
...
MXL_STATUS MxL_GetDemodStatisticCounters(PMXL_DEMOD_STAT_COUNT_T StatCountPtr)
{
  ...
  for (i = 0; i < MAX_241SF_DEVICES; i++)
  {
    if (AccStatCounter[i].I2cSlaveAddr == StatCountPtr->I2cSlaveAddr) break;
  }
