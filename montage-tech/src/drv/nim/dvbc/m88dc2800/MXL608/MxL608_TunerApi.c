...
const UINT8 MxLWare608DrvVersion[] = {1, 1, 1, 7, 0}; 
                                          ...
                                          UINT8 standbyLt)
{
  ...
  switch(powerMode)
  {
    ...
    case MXL608_PWR_MODE_STANDBY:
      ...
      {
        status |= MxLWare608_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x01);
        status |= MxLWare608_OEM_WriteRegister(devId, DFE_SEQ_TUNE_RF1_BO_REG, (UINT8)(standbyLt & 0x3F));
        status |= MxLWare608_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x00);
      }	
      break;
    ...
    if (chanTuneCfg.signalMode <= MXL608_DIG_J83B) 
    {
      ...
      switch(chanTuneCfg.bandWidth)
      {
        ...
        case MXL608_TERR_BW_8MHz:
            status |= MxLWare608_OEM_WriteRegister(devId, CHAN_TUNE_BW_REG, (UINT8)chanTuneCfg.bandWidth);
