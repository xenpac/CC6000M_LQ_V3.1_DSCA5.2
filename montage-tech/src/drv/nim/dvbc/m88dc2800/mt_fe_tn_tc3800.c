 ...
 * Modify           0.01.25     2015.09.24      Lei.Zhu
 *----------------------------------------------------------------------------
 *****************************************************************************/
...
void mt_fe_tn_init_tc3800(MT_FE_Tuner_Handle_TC3800 handle)
{
	if (handle->tuner_init_OK != 1)
	{
		...
		//handle->tuner_mode				 = 0;
		handle->lt_gain_fix				 = 0;
		handle->lt_gain_code			 = 9;
		...
		handle->tuner_version			 = 125;
		handle->tuner_time				 = 15092414;
...
void mt_fe_tn_set_freq_tc3800(MT_FE_Tuner_Handle_TC3800 handle, U32 freq_KHz)
{
	...
	if(buf == 0x0e)
	{
		...
		_mt_fe_tn_set_reg_bit_tc3800(handle, 0x60, buf, 3, 1);
	}
	else
	{
		buf = (U8)(buf >> 1);
	...
	if((handle->tuner_mode == 1) || (handle->tuner_mode == 2))
	{
		...
		if(buf == 0xff)
		{
			if(handle->lt_gain_fix == 1)
			{
				buf = handle->lt_gain_code;
				buf = (U8)((buf << 4) + 5);
				_mt_fe_tn_set_reg_tc3800(handle, 0x14, buf);
			}
			else
			{
...
extern U8 have_extra_iVppSel_iGainStep;
extern U8 extra_iVppSel;
extern U8 extra_iGainStep;
extern MT_FE_DC2800_SUPPORTED_TUNER dc2800_tuner_type ;


void tuner_attach_tc3800_for_wasu(void)
{
    have_extra_iVppSel_iGainStep = 1;
    extra_iVppSel = 1;
    extra_iGainStep = 2;
