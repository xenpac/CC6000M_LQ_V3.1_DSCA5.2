...
MT_FE_DC2800_Device_Handle R836_handle = NULL;
...
int I2C_Write(I2C_TYPE *I2C_Info)
{
	//implement your I2C code here//
	UINT8 p_buf[2];

	p_buf[0] = I2C_Info->RegAddr;
	p_buf[1] = I2C_Info->Data;

	R836_handle->tn_write(R836_handle, p_buf, (unsigned short)2);
    //R836_handle->tn_set_reg(R836_handle, p_buf[0], p_buf[1]);
...
int I2C_Write_Len(I2C_LEN_TYPE *I2C_Info)
{
 ...
 //	UINT8 p_buf[128];
	U8 i = 0;
       ...
       I2C_TYPE I2C_Info_2 = {0};

//	p_buf[0] = I2C_Info->RegAddr;
	for(i = 0; i < I2C_Info->Len; i ++)
	{
           I2C_Info_2.RegAddr = I2C_Info->RegAddr + i;
           I2C_Info_2.Data = I2C_Info->Data[i];
           I2C_Write(&I2C_Info_2);
          // p_buf[i + 1] = I2C_Info->Data[i];
	}

    //R836_handle->tn_write(R836_handle, p_buf, (unsigned short)(I2C_Info->Len + 1));
#endif
    UINT8 p_buf[128];
...
int I2C_Read_Len(I2C_LEN_TYPE *I2C_Info)
{
	//implement your I2C code here//
	/*
	int DataCunt = 0;
	...
	*/
	...
	R836_handle->tn_read(R836_handle, p_buf, (unsigned short)(I2C_Info->Len));

	for(i = 0; i < I2C_Info->Len; i ++)
	{
		I2C_Info->Data[i] = R836_Convert(p_buf[i]);
	}
...
{
	R836_handle->mt_sleep(ms);
}

Sys_Info_Type R840_Sys_Sel(R840_Standard_Type R840_Standard)
{
	Sys_Info_Type R840_Sys_Info = {0};
...
Freq_Info_Type R840_Freq_Sel(UINT32 LO_freq, UINT32 RF_freq, R840_Standard_Type R840_Standard)
{
	Freq_Info_Type R840_Freq_Info = {0};
...
SysFreq_Info_Type R840_SysFreq_Sel(R840_Standard_Type R840_Standard,UINT32 RF_freq)
{
	SysFreq_Info_Type R840_SysFreq_Info = {0};
...
else  //R836
{
	 ...
	 else //DTV
	 {
		 //Hys = 1.4+/-0.6
		 R840_I2C.RegAddr = 0x06;
		 R840_Array[6] = (R840_Array[6] & 0xFB) | 0x00;  //[2]=0
		 R840_I2C.Data = R840_Array[6];
		 if(I2C_Write(&R840_I2C) != RT_Success)
			 return RT_Fail;
...
MT_FE_RET mt_fe_tn_init_R836(void *dev_handle)
{
	R836_handle = (MT_FE_DC2800_Device_Handle)dev_handle;
	R840_IMR_done_flag = FALSE;
...
MT_FE_RET mt_fe_tn_set_freq_R836(void *dev_handle, U32 freq_KHz, U16 symbol_rate_KSs)
{
	R840_Set_Info R840_Info;
	R836_handle = (MT_FE_DC2800_Device_Handle)dev_handle;
	...
	R840_Info.RF_KHz = freq_KHz; // unit: kHz
	//Loop Through
	...
	if(R840_SetPllData(R840_Info) != RT_Success)
		return MtFeErr_Fail;
	else
		return MtFeErr_Ok;
}
...
MT_FE_RET mt_fe_tn_get_signal_strength_R836(void *dev_handle, U32 *p_gain, U32 *p_strength)
{
	R836_handle = (MT_FE_DC2800_Device_Handle)dev_handle;
...
void mt_fe_tn_wakeup_R836(void *dev_handle)
{
    R836_handle = (MT_FE_DC2800_Device_Handle)dev_handle;
    R840_WakeUp();
}

void mt_fe_tn_sleep_R836(void *dev_handle)
{
    R836_handle = (MT_FE_DC2800_Device_Handle)dev_handle;
     R840_Standby(LT_OFF);
}
