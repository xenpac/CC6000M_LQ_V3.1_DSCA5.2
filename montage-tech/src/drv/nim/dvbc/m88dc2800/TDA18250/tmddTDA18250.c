...
UInt8 uTimeTransition[] = {0,2,3,5,9,18,36,224};
UInt16 uTimeTransitionAGC1[] = {0,3,5,9,17,33,65,401};
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        if (err == TM_OK)
        {
            ...
            pObj->I2CMap.uBx3B.RCCAL_delay       = 0x00;
            ...
            pObj->I2CMap.uBx41.IF_byte_1         = 0x02;
...
/* RETURN:      TM_OK if no error                                             */
/*                                                                            */
/* NOTES:       000: 92dBm?11: 102dBm (step2dBm)                             */
/*============================================================================*/
tmErrorCode_t
...
{
   //#define TDA18250_IMAGE_CALIB_WAIT 10
  // #define TDA18250_IMAGE_CALIB_TIMEOUT	50
    ptmTDA18250Object_t   pObj = Null;
    tmErrorCode_t         err = TM_OK;
    UInt8                 FlagMSM;
    UInt32                ulTimeout = 0;
...
{
  // #define TDA18250_SIGNAL_CALIB_WAIT 10
  //  #define TDA18250_SIGNAL_CALIB_TIMEOUT	50
    ptmTDA18250Object_t   pObj = Null;
    tmErrorCode_t         err = TM_OK;
    UInt8                 FlagMSM;
    UInt32                ulTimeout = 0;
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        for (FreqCpt=0; FreqCpt<3; FreqCpt++)
        {
            if (err == TM_OK)
            {
                err = TDA18250SignalCalibration(tUnit, (UInt8)(FreqCpt + 1), uIrFreq[FreqCpt]);
...
/*============================================================================*/
tmErrorCode_t
TDA18250Reset(tmUnitSelect_t  tUnit   )  /* I: Unit number */
{
    ptmTDA18250Object_t   pObj = Null;
    tmErrorCode_t         err = TM_OK;
    UInt8                 uIdentity ;
    ...
    if (err == TM_OK)
    {
        ...
        if (err == TM_OK)
        {
            err = tmddTDA18250SetHP_MTO_T_CST(tUnit, (UInt8)pObj->MTO_TimeCst);
...
{
 //  #define TDA18250_MUTEX_TIMEOUT 	10
    ptmTDA18250Object_t pObj = Null;
    tmErrorCode_t       err  = TM_OK;
    UInt8               AGC2_Gain;
...
(
    tmUnitSelect_t tUnit  ,  /* I: Unit number */tmTDA18250StandardMode_t    StandardMode    /* I: Standard mode of this device */
)
{
    ptmTDA18250Object_t pObj = Null;
    tmErrorCode_t       err  = TM_OK;
    ...
    if (err == TM_OK)
    {
        ...
        if (err == TM_OK)
        {
            err = tmddTDA18250SetLP_Fc(tUnit, (UInt8)pObj->Std_Array[pObj->StandardMode].LP_Fc);
...
)
{
    ...
    if (err == TM_OK)
    {
        ...
        if (err == TM_OK)
        {
            ...
            {
                err = TDA18250WriteAGC2_UpDownGain(tUnit, uGudAdd, (UInt8)(pObj->AGC2_GdownDetector[uBPIndex][uGudAdd - 1] + 1));
    ...
    if (err == TM_OK)
    {
        ...
        if (uState==tmTDA18250_ON)
        {
            ...
            while ((uValue!=0)&&(uCpt<=20))
            {
                if (err == TM_OK)
                {
                    err = TDA18250Wait(pObj, uTimeTransitionAGC1[(pObj->AGC1_TimeCst)]);  /* wait */
                    tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "TDA18250Wait(0x%08X) failed.", tUnit));
                }
                if (err == TM_OK)
                {
    ...
    if (err == TM_OK)
    {
        ...
        if (err == TM_OK)
        {
            ...
            while ((RdSwitchState != SwitchState) && (uCpt < 5))
            {
                ...
                if (err == TM_OK)
                {
                    err = TDA18250Wait(pObj, uTimeTransition[pObj->Switch_TimeCst]);
                    tmASSERTExT(err, TM_OK, (DEBUGLVL_ERROR, "TDA18250Wait(0x%08X) failed.", tUnit));
                }
