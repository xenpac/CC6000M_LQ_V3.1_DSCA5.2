...
extern DMD_ERROR_t DMD_set_t2_work_profile(DMD_PARAMETER_t* param); //20131219, set work mode for MN88473
...
extern DMD_ERROR_t DMD_set_dmd_control_tuner_i2c_mode(void);//20131219, for MN88473
...
extern DMD_ERROR_t DMD_device_dvbt2_read_P1_flag(  DMD_u8_t* p1flg_rd ); //troy, 20130131, to judge signal including FEF or not 
...
extern void DMD_increase_compatibility_with_nonstandard_signal(DMD_PARAMETER_t *param__UNUSED);//20130626, troy.wang added, for non-standard signal with high ratio dummy cell inside.
...
extern DMD_ERROR_t DMD_Constellation_monitor_pre_setting( DMD_PARAMETER_t* param, DMD_u8_t action, DMD_DVBT2_CON_MON_t dvbt2_mon_type, DMD_DVBT_CON_MON_t dvbt_mon_type);//20131015
extern DMD_ERROR_t DMD_Constellation_monitoring( DMD_PARAMETER_t* param, DMD_u32_t CarrierNo,DMD_s32_t* p_IMON_RD, DMD_s32_t* p_QMON_RD_p);//20131015

extern void DMD_DVBT_lock_high_noise_signal(void);//20131211, for GIEC factory signal(also related with hardware design)
extern DMD_ERROR_t DMD_device_read_chipid( DMD_u8_t*	data );
extern DMD_u32_t	DMD_RegSet_Rev;
extern DMD_ERROR_t DMD_get_check_lock( DMD_PARAMETER_t *param );
