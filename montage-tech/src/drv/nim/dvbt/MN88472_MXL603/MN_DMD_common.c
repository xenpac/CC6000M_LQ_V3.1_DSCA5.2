...
DMD_ERROR_t	DMD_I2C_MaskWrite(  DMD_u8_t	slvadr , DMD_u8_t adr , DMD_u8_t mask , DMD_u8_t  data )
{
	DMD_u8_t	rd;
	DMD_ERROR_t ret;
...
DMD_u32_t	DMD_calc_SQI( DMD_PARAMETER_t* param)
{
	...
	else
	{
		...
		{
			DMD_DEBUG(" --- DVB-T SQI   --- ");
			DMD_DEBUG(" cnr = %d,  cnr_rel = %d, bit error =  %d / %d \n ", cnr,cnr_rel,param->info[DMD_E_INFO_BERRNUM],param->info[DMD_E_INFO_BITNUM]);
			DMD_DEBUG(" constallation_t = %d, coderate_t= %d \n",mod, cr);
		}
		else if ( param->system == DMD_E_DVBT2 )
		{
			DMD_DEBUG(" --- DVB-T2 SQI --- ");
			DMD_DEBUG(" cnr = %d, cnr_rel = %d, bit error =  %d / %d \n ", cnr,cnr_rel, param->info[DMD_E_INFO_BERRNUM],param->info[DMD_E_INFO_BITNUM]);
			DMD_DEBUG(" constallation_t2 = %d, coderate_t2 = %d \n",mod, cr);
		}		
...
#ifdef COMMON_DEBUG_ON 
	DMD_DEBUG(" ---  SQI_done  :  cnr_rel = %d, beri = %d, ber_sqi = %d, sqi = %d --- ",cnr_rel, beri, ber_sqi, sqi/100 ); //20130129
#endif 
	return sqi / CNR_MULT;
}
...
DMD_ERROR_t DMD_trans_reg(  DMD_I2C_Register_t* t)
{
	int i=0;
 ...
 // For example, if write DMD_BANK_T2_, the slave ID should be (0x1c << 1). 
		...
		case 0x1a:
			...
			break;
		default:
      DMD_DBG_TRACE("DMD_trans_reg slv[%x],idx[%d]\n",t[i].slvadr,i);
			return DMD_E_ERROR;
		}
		if( DMD_I2C_Write( slvadr , t[i].adr , t[i].data ) == DMD_E_ERROR ){
      DMD_DBG_TRACE("DMD_trans_reg idx[%d]slv[%x],adr[%x] data[%x]\n",i,t[i].slvadr,t[i].adr,t[i].data);
			return DMD_E_ERROR;
		}
		i++;
...
/* Register  */
/* **************************************************** */
DMD_ERROR_t DMD_send_registers(  DMD_u8_t	*slaveset , DMD_u8_t* regset )
{
	DMD_u32_t	i;
	for(i=0;;)
	{
...
DMD_ERROR_t DMD_read_registers(void)
{
       DMD_u32_t        i;
       DMD_u8_t rd;
	   //DMD_s8_t	buf[10];
       ...
       for(i=0;;i++)
       {			   
               DMD_I2C_Read( (DMD_BANK_T2_), i, &rd );
               DMD_DBG_TRACE("DVB-T2 : Regaddr[0x%x]  =  0x%x     \n",i,rd);
        ...
        for(i=0;;i++)
       {              
               DMD_I2C_Read( (DMD_BANK_T_), i, &rd );
               DMD_DBG_TRACE("DVB-T Regaddr[0x%x]  =  0x%x     \n",i,rd);
...
DMD_text_t	DMD_value_decimal( DMD_u32_t val )
{
	...
	sprintf( buf , "DEC %d",  val ); //this function is included in <stdio.h>
	return buf;
}
DMD_text_t	DMD_value_hex( DMD_u32_t val )
{
	static DMD_s8_t	buf[256];
	sprintf( buf ,  "HEX 0x%x",  val );
	return buf;
}
...
DMD_text_t DMD_info_value_dvbt2( DMD_u32_t id , DMD_u32_t val )
{
	switch( id )
	{
		...
		case   DMD_E_INFO_DVBT2_L1_POST_SCRAMBLE_ENABLED_FLG :	return	DMD_value_decimal(val);	//20131219, add for indicator of L1_POST_SCRAMBLE
#endif
		default:
			return "Not Defined";
...
DMD_textlist_t	DMD_TEXTLIST_DVBT2_COD		=	{	8	,{	"1/2",	"3/5",	"2/3",	"3/4",	"4/5(reserved)",	"5/6(reserved)",	"1/3",	"2/5"}};//add T2-lite CR in the "()", 20131219	
