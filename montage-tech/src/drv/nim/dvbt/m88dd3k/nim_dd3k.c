...
static RET_CODE nim_m88dd3k_get_total_packages(lld_nim_t *p_lld,
                                           u32 *p_total_packages)
{
  nim_dd3k_priv_t *p_priv = (nim_dd3k_priv_t *)(p_lld->p_priv);
  U8 reg_1c0 = 0;
  U8 reg_1c1 = 0;
  ...
  MT_FE_DD_Device_Handle dd3k_handle = p_priv->mt_dd_handle;

  dd3k_handle->dmd_get_reg(dd3k_handle, 0x01c2, &reg_1c2);
  tmp = (u32)(reg_1c2 << 16);
  dd3k_handle->dmd_get_reg(dd3k_handle, 0x01c1, &reg_1c1);
  tmp += (u32)(reg_1c1 << 8);
  dd3k_handle->dmd_get_reg(dd3k_handle, 0x01c0, &reg_1c0);
  tmp += (u32)reg_1c0;

  *p_total_packages = tmp;

  DS3K_PRINT("%s total_packets[%d]\n", __FUNCTION__, *p_total_packages);
  
  return SUCCESS;
}

static RET_CODE nim_m88dd3k_get_error_packages(lld_nim_t *p_lld,
                                           u32 *p_err_package)
{
  nim_dd3k_priv_t *p_priv = (nim_dd3k_priv_t *)(p_lld->p_priv);
  U8 reg_a3 = 0;
  ...
  dd3k_handle->dmd_get_reg(dd3k_handle, 0x0a4, &reg_a4);
  tmp = (u32)(reg_a4 << 8);
  dd3k_handle->dmd_get_reg(dd3k_handle, 0x0a3, &reg_a3);
  tmp += (u32)reg_a3 ;

  *p_err_package = tmp;

  DS3K_PRINT("%s err_packets[%d]\n", __FUNCTION__, p_err_package);
...
static RET_CODE nim_m88dd3k_ioctrl(lld_nim_t *p_lld, u32 cmd, u32 param)
{
  ...
  switch(cmd)
  {
    ...
    case NIM_IOCTRL_GET_TOTAL_PACKAGES:
	  nim_m88dd3k_get_total_packages(p_lld, &param);
	  break;
    case NIM_IOCTRL_GET_ERR_PACKAGES:
	  nim_m88dd3k_get_error_packages(p_lld, &param);
	  break;
...
RET_CODE nim_m88dd3k_open(lld_nim_t *p_lld, void *cfg)
{
  ...
  memset(&tuner_ct_cfg, 0, sizeof(MT_FE_DD_TN_DEV_SETTINGS));
  memset(&tuner_dc_cfg, 0, sizeof(MT_FE_DD_TN_DEV_SETTINGS));
  ...
  else if(p_priv->cfg.tn_version == TC3800)
  {
    ...
    tuner_ct_cfg.tuner_sleep=(S32 (*)(void *))mt_fe_tn_sleep_tc3800_tc;
    tuner_ct_cfg.tuner_wake_up=(S32 (*)(void *))mt_fe_tn_wake_up_tc3800_tc;
    ...
    tuner_dc_cfg.tuner_sleep=(S32 (*)(void *))mt_fe_tn_sleep_tc3800_tc;
    tuner_dc_cfg.tuner_wake_up=(S32 (*)(void *))mt_fe_tn_wake_up_tc3800_tc;
  }
#if 0
