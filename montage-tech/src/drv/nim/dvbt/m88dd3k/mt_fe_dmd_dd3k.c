...
*	Modify			2.80.10		2015.07.28		WangBingju
********************************************************************************************************/
...
MT_FE_RET mt_fe_demod_settings_default_dd3k(MT_FE_DD_Device_Handle  handle)
{
	...
	handle->demod_ct_settings.ts_out_mode = MtFeTsOutMode_Parallel;
	...
	handle->demod_dc_settings.ts_out_mode = MtFeTsOutMode_Parallel;
...
MT_FE_RET mt_fe_config_tuner_settings_dd3k(MT_FE_DD_Device_Handle handle, MT_FE_DD_TN_DEV_SETTINGS *tuner_handle)
{
	...
	if (tuner_handle->tuner_mode == MtFeType_CTTB)
	{
		...
		handle->tuner_ct_settings.tuner_sleep = tuner_handle->tuner_sleep;
        handle->tuner_ct_settings.tuner_wake_up = tuner_handle->tuner_wake_up;
	}
	else
	{
        ...
        handle->tuner_dc_settings.tuner_sleep = tuner_handle->tuner_sleep;
        handle->tuner_dc_settings.tuner_wake_up = tuner_handle->tuner_wake_up;
	}
...
MT_FE_RET _mt_fe_dmd_reg_init_dd3k_t(MT_FE_DD_Device_Handle  handle)
{
	...
	else if(handle->demod_id == MtFeDmdId_DD630X)
	{
        ...
        handle->dmd_set_reg(handle, 0x00c2, 0x2f);
		...
		handle->dmd_set_reg(handle, 0x0160, 0xfc);
        handle->dmd_set_reg(handle, 0x0185, 0x0e);
		handle->dmd_set_reg(handle, 0x0189, 0x24);
		handle->dmd_set_reg(handle, 0x0291, 0x83);
        handle->dmd_set_reg(handle, 0x0290, 0x80);

		handle->dmd_set_reg(handle, 0x0148, 0x44);
		handle->dmd_set_reg(handle, 0x0149, 0x44);
		handle->dmd_set_reg(handle, 0x014a, 0x44);
		handle->dmd_set_reg(handle, 0x014c, 0x72);
		...
		handle->dmd_set_reg(handle, 0x0160, 0x7c);
	...
	*p_state = MtFeLockState_Waiting;
		...
		if(handle->demod_cur_mode != MtFeType_DVBC)
		{
			...
			{/*	Unlock	*/
				*p_state = MtFeLockState_Unlocked;
				mt_fe_print(("MT:	FEC Unlock!!  [0xc2 = 0x%x]\n", value));
				return MtFeErr_Ok;
			}
			else
	...
	else if(handle->demod_id == MtFeDmdId_DD630X)
	{
		if(handle->demod_cur_mode != MtFeType_DVBC)
		{
			...
			if ((value & 0x01) == 0x01)
			{
			*p_state = MtFeLockState_Locked;/*	Unlock	*/
			}
			else
			{/*	Unlock	*/
				*p_state = MtFeLockState_Unlocked;
...
//	U32		ch_info_code = 0;
	...
	m = 0;
//	mt_fe_dmd_get_chan_info_code_dd3k(handle, &ch_info_code);
...
**
*/
 MT_FE_RET _mt_fe_dmd_get_ber_dd3k_c(MT_FE_DD_Device_Handle  handle,DB *dvbc_ber)
{
	static U32 ber = 0;
	U16	tmp = 0;
	U8	reg_a0 = 0;
	...
	if ((reg_a0 & 0x80) != 0x80)
	{
		...
		tmp = tmp+(U16)(reg_a1);
		ber = tmp;
		ber /= 33554432;					/*	(2^(2*5+12))*8		*/

		handle->dmd_set_reg(handle, 0x00a0, 0x05);
		handle->dmd_set_reg(handle, 0x00a0, 0x85);
	}
...
MT_FE_RET mt_fe_dmd_get_quality_dd3k_t(MT_FE_DD_Device_Handle  handle, S8 *p_snr)
{
	...
	*p_snr = 0;
	...
	if(dd3k_state==MtFeLockState_Locked)
	{
		...
		*p_snr = _mt_fe_dmd_acc_2_snr_dd3k_t(handle, noise);/*p_snr is %*/
	}
	else
		*p_snr = 0;
