...
const UINT8 MxLWare603DrvVersion[5] = {2, 1, 3, 2, 0};
                                                  ...
                                                  MXL_BOOL singleSupply_3_3V)
{
  ...
  MxLWare603_OEM_ReadRegister(devId, 0x00, &readData);
...
MXL_STATUS MxLWare603_API_CfgDevXtal(UINT8 devId, MXL603_XTAL_SET_CFG_T xtalCfg)
{
  ...
  if ((xtalCfg.xtalFreqSel == MXL603_XTAL_16MHz) || (xtalCfg.xtalFreqSel == MXL603_XTAL_24MHz))
  {
    ...
    if (xtalCfg.XtalSharingMode == MXL_ENABLE)
    {
      ...
      status |= MxLWare603_OEM_WriteRegister(devId, XTAL_EXT_BIAS_REG, 0x80);
    }
    else
    {
      control &= 0x01;
      // program Clock out div & Xtal sharing
      status |= MxLWare603_OEM_WriteRegister(devId, XTAL_ENABLE_DIV_REG, control);
      status |= MxLWare603_OEM_WriteRegister(devId, XTAL_EXT_BIAS_REG, 0x0A);
    }
...
MXL_STATUS MxLWare603_API_CfgDevPowerMode(UINT8 devId,
                                          ...
                                          UINT8 standbyLt)
{
  ...
  switch(powerMode)
  {
    ...
    case MXL603_PWR_MODE_ACTIVE:
      ...
      status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x01);
      if (enableLoopthrough == MXL_ENABLE)
        status |= MxLWare603_OEM_WriteRegister(devId,DFE_SEQ_TUNE_RF1_BO_REG,0x0E);
      else
        status |= MxLWare603_OEM_WriteRegister(devId,DFE_SEQ_TUNE_RF1_BO_REG,0x37);
      status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x00);
      break;
    ...
    case MXL603_PWR_MODE_STANDBY:
      ...
      {
        status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x01);
        status |= MxLWare603_OEM_WriteRegister(devId,DFE_SEQ_TUNE_RF1_BO_REG,(standbyLt & 0x3F));
        status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x00);
      }
      break;
...
MXL_STATUS MxLWare603_API_ReqDevPllState(UINT8 devId, MXL603_PLL_STATE_E* PllStatePtr)
{
  ...
  *PllStatePtr = MXL603_PLL_STATE_NA;
  ...
  for (k = 0; k < 7; k++)
  {
    ...
    status |= MxLWare603_OEM_ReadRegister(devId, regAddr[k], &regData[k]);
  }
...
MXL_STATUS MxLWare603_API_CfgTunerLoopThrough(UINT8 devId, MXL_BOOL loopThroughCtrl, UINT8* gainArray, UINT8* attArray)
{
  UINT8 status = MXL_SUCCESS;
  UINT8 regData;
  ...
  if (loopThroughCtrl <= MXL_ENABLE)
  {
    ...
    status |= MxLWare603_OEM_ReadRegister(devId, DIG_ANA_GINJO_LT_REG, &regData);

    if (loopThroughCtrl == MXL_ENABLE)
      regData |= 0x10;  // Bit<4> = 1
    else
      regData &= 0xEF;  // Bit<4> = 0
    status |= MxLWare603_OEM_WriteRegister(devId, DIG_ANA_GINJO_LT_REG, regData);

    if ((loopThroughCtrl == MXL_ENABLE) && (gainArray != NULL) && (attArray != NULL))
    {
      regData = (gainArray[3]<<6 ) | (gainArray[2] <<4 ) | (gainArray[1] <<2 ) | gainArray[0] ;
      status |= MxLWare603_OEM_WriteRegister(devId, DFE_SEQ_DIGANA_LT_GAIN_REG, regData);
      regData = (attArray[3]<<6 ) | (attArray[2] <<4 ) | (attArray[1] <<2 ) | attArray[0] ;
      status |= MxLWare603_OEM_WriteRegister(devId, DFE_SEQ_DIGANA_LT_ATTN_REG, regData);
    }
    status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x00);
  }
  else
...
MXL_STATUS MxLWare603_API_CfgTunerSouthAfricaLT(UINT8 devId)
{
  UINT8 status = MXL_SUCCESS;
  ...
  status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x01);
  status |= MxLWare603_OEM_WriteRegister(devId, DFE_SEQ_DIGANA_LT_GAIN_REG, 0x95);
  status |= MxLWare603_OEM_WriteRegister(devId, DFE_SEQ_DIGANA_LT_ATTN_REG, 0x00);
  status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x00);
