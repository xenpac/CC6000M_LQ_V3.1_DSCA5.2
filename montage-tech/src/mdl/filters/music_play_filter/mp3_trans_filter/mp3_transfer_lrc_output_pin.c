...
static mp3_trans_lrc_out_pin_private_t * get_priv(handle_t _this)
{
  CHECK_FAIL_RET_NULL(NULL != _this);
  return &((mp3_trans_lrc_out_pin_t *)_this)->private_data;
}

static RET_CODE mp3_trans_lrc_out_pin_decide_buffer_size(handle_t _this)
{
  mp3_trans_lrc_out_pin_private_t *p_priv = get_priv(_this);
...
mp3_trans_lrc_out_pin_t * mp3_trans_lrc_out_pin_create(mp3_trans_lrc_out_pin_t *p_pin, 
                                                                interface_t *p_owner)
{
  mp3_trans_lrc_out_pin_private_t *p_priv = NULL;
  ipin_t *p_ipin = NULL;
  ...
  transf_pin_para.p_name = "mp3_trans_lrc_o_pin";
  transf_output_pin_create(&p_pin->m_pin, &transf_pin_para);

  //init private date
  p_priv = &(p_pin->private_data);
  memset(p_priv, 0x0, sizeof(mp3_trans_lrc_out_pin_private_t));
  p_priv->p_this = p_pin;
  ...
  p_output_pin->decide_buffer_size = mp3_trans_lrc_out_pin_decide_buffer_size;
