...
#define CAS_SYSTEM_TIME_SIZE (sizeof(utc_time_t))
        ...
        if (service_id == p_this->svc_id[0])
        {
          ...
          if (NULL != p_this->p_policy->update_ecm_info)
          {
            if (p_this->p_policy->update_ecm_info(param.p_buffer, cas_ecm_info, (u16 *)&ecm_num))
            {
              ...
              OS_PRINTF("@@@on_dvb_msg  send ecm update evt to ui !!!!!!!! \n ");
              cas_send_evt_to_ui(CAS_EVT_ECM_UPDATE, 0, 0);
            }
          }
        }
      }
...
static void cas_process_msg(handle_t handle, os_msg_t *p_msg)
{
  ...
  switch(p_msg->content)
  {
    ...
    case CAS_CMD_IPPV_INFO_GET:
      ...
      ((ipps_info_t *)temp_addr)->operator_id = p_msg->para1;
      cam_id = (p_msg->para2 & 0xFF);
      slot = ((p_msg->para2 >> 16) & 0xFF);
      cas_param.cam_id = cam_id;
      cas_param.p_param = (void *)temp_addr;
    ...
    case CAS_CMD_CARD_INFO_GET:
      ...
      break;
	case CAS_CMD_SYSTEM_TIME_GET:
      temp_addr = cas_get_mem_addr(handle, CAS_SYSTEM_TIME_ID, CAS_SYSTEM_TIME_SIZE);
      OS_PRINTF("get mem addr 0x%x, LINE=%d\n", temp_addr,__LINE__);
      cam_id = (p_msg->para2 & 0xFF);
      slot = ((p_msg->para2 >> 16) & 0xFF);
      cas_param.cam_id = cam_id;
      cas_param.p_param = (void *)temp_addr;
      ret = cas_io_ctrl(slot, CAS_IOCMD_SYSTEM_TIME_GET, &cas_param);
      cas_send_evt_to_ui(CAS_EVT_SYSTEM_TIME_INFO, (u32)ret, temp_addr);
      break;
    ...
    case CAS_CMD_MON_CHILD_STATUS_GET:
      ...
      break;
      case CAS_CMD_MON_CHILD_STOP_FEED:
      cam_id = (p_msg->para2 & 0xFF);
      slot = ((p_msg->para2 >> 16) & 0xFF);
      cas_param.cam_id = cam_id;
      cas_param.p_param = (void *)p_msg->para1;
      ret = cas_io_ctrl(slot, CAS_IOCMD_MON_CHILD_FEED_STOP, &cas_param);
      break;	    
...
case CAS_C_STB_URGENT_BROADCAST:
      ...
      case CAS_C_STB_NOTIFICATION:
        ...
        evt.data2 = event.param;
        break;
    case CAS_C_SHOW_URGENT_MSG:
        OS_PRINTF("cas urgent msg, LINE=%d\n", __LINE__);
        evt.id = CAS_EVT_SHOW_URGENT_MSG;
        evt.data1 = (event.slot_id << 16) | cas_param.cam_id;
        evt.data2 =  event.param;
        break;        
    case CAS_C_HIDE_URGENT_MSG:
        OS_PRINTF("cas hide urgent msg, LINE=%d\n", __LINE__);
...
static void init(handle_t handle)
{
  ...
  MT_ASSERT(TRUE == mtos_sem_create(&p_this->evt_mutex, TRUE));
