...
static dm_dmh_info_t *p_all_dmh_info = NULL;
...
static void ui_ota_reset_block_data(u8 block_id)
{
  ...
  for(i = 0;i < p_all_dmh_info->header_num;i ++)
  {
     for(j = 0; j < p_all_dmh_info->header_group[i].block_num; j++)
      {
         if((p_all_dmh_info->header_group[i].dm_block[j].id == block_id)
            && ((p_all_dmh_info->header_group[i].dm_block[j].base_addr % CHARSTO_SECTOR_SIZE) == 0)
            && ((p_all_dmh_info->header_group[i].dm_block[j].size % CHARSTO_SECTOR_SIZE) == 0)
            && (p_all_dmh_info->header_group[i].dm_block[j].base_addr != 0)
            && (p_all_dmh_info->header_group[i].dm_block[j].size != 0))
          {
            block_addr = p_all_dmh_info->header_group[i].dm_block[j].base_addr;
            block_size = p_all_dmh_info->header_group[i].dm_block[j].size;
...
#else
   ota_param.ota_tp.lock_mode = SYS_DVBC;
   ota_param.ota_tp.lockc.tp_freq = 307000;
   ota_param.ota_tp.lockc.tp_sym = 6875;
   ota_param.ota_tp.lockc.nim_modulate = NIM_MODULA_QAM64;
   ota_param.ota_tp.data_pid = 0x1b58;
#endif
       ...
       for(i = 0;i < p_all_dmh_info->header_num;i ++)
        {
           for(j = 0; j < p_all_dmh_info->header_group[i].block_num; j++)
            {
               if((p_all_dmh_info->header_group[i].dm_block[j].id != OTA_ALL_MAIN_CODE_BLOCK_ID)
                  && ((p_all_dmh_info->header_group[i].dm_block[j].base_addr % CHARSTO_SECTOR_SIZE) == 0)
                  && ((p_all_dmh_info->header_group[i].dm_block[j].size % CHARSTO_SECTOR_SIZE) == 0)
                  && (p_all_dmh_info->header_group[i].dm_block[j].base_addr != 0)
                  && (p_all_dmh_info->header_group[i].dm_block[j].size != 0))
                {
                        for(k= 1;k < OTA_DM_MAX_BLOCK_NUMBER;k++)
                        {
                          if(ota_param.block_table[k].exist_bit != TRUE)
                            {
                              ota_param.block_table[k].id = p_all_dmh_info->header_group[i].dm_block[j].id;
                              ota_param.block_table[k].type = p_all_dmh_info->header_group[i].dm_block[j].type;
                              ota_param.block_table[k].base_addr = p_all_dmh_info->header_group[i].dm_block[j].base_addr;
                              ota_param.block_table[k].size = p_all_dmh_info->header_group[i].dm_block[j].size;
...
static BOOL ui_ota_api_do_dsi_check(ota_dsi_info_t *p_info)
{
  ...
  #else
    mul_ota_dm_api_dm_read(MISC_OPTION_BLOCK_ID,0,
                               sizeof(misc_options_t), (u8 *)&misc);
  #endif
   memset(&ota_param,0,sizeof(ota_param_t));
...
void ui_ota_api_config(BOOL g_new_mem)
{
  ...
  for(i = 0;i < p_all_dmh_info->header_num;i ++)
  {
            ...
            && (p_all_dmh_info->header_group[i].dm_block[j].size != 0))
          {
                  for(k= 1;k < OTA_DM_MAX_BLOCK_NUMBER;k++)
                  {
                    if(ota_config.sys_block[k].exist_bit != TRUE)
                      {
                        ota_config.sys_block[k].id = p_all_dmh_info->header_group[i].dm_block[j].id;
                        ota_config.sys_block[k].type = p_all_dmh_info->header_group[i].dm_block[j].type;
                        ota_config.sys_block[k].base_addr = p_all_dmh_info->header_group[i].dm_block[j].base_addr;
                        ota_config.sys_block[k].size = p_all_dmh_info->header_group[i].dm_block[j].size;
                        ota_config.sys_block[k].dm_offset = p_all_dmh_info->header_group[i].dm_offset;
                        ...
                        ota_config.sys_block[k].crc = p_all_dmh_info->header_group[i].dm_block[j].crc;
   ...
   ota_config.tri_mode = bl_info.ota_status;
      ...
      ||(TRUE == ota_config.destroy_flag)) 
  {
   ...
   OS_PRINTF("#####@@@@@debug check:para.ota_config.ota_version_check :0x%x\n",
                      (u32)ota_config.ota_version_check);

    read_buffer_size = CHARSTO_SIZE * 2 + 64 * KBYTES;
   ...
   if(policy.run_select == OTA_WORK_SELECT_AUTO)
    {
       ...
       memcpy(&policy.tp_info.ota_set_back_tp.lockt,
                          ...
                          */
       policy.tp_info.ota_set_tp.lock_mode = ota_param.ota_tp.lock_mode;
       policy.tp_info.ota_set_tp.data_pid = ota_param.ota_tp.data_pid;
       memcpy(&policy.tp_info.ota_set_tp.lockc,&ota_param.ota_tp.lockc,sizeof(dvbc_lock_info_t));
       memcpy(&policy.tp_info.ota_set_tp.locks,&ota_param.ota_tp.locks,sizeof(dvbs_lock_info_t)); 
       memcpy(&policy.tp_info.ota_set_tp.lockt,&ota_param.ota_tp.lockt,sizeof(dvbt_lock_info_t));

       policy.tp_info.ota_set_back_tp.lock_mode = ota_param.ota_tp.lock_mode;
       policy.tp_info.ota_set_back_tp.data_pid = ota_param.ota_tp.data_pid;
       memcpy(&policy.tp_info.ota_set_back_tp.lockc,
                        &ota_param.ota_tp.lockc,
                        sizeof(dvbc_lock_info_t));
       memcpy(&policy.tp_info.ota_set_back_tp.locks,
                        &ota_param.ota_tp.locks,
                        sizeof(dvbs_lock_info_t)); 
       memcpy(&policy.tp_info.ota_set_back_tp.lockt,
                          &ota_param.ota_tp.lockt,
                          sizeof(dvbt_lock_info_t));
...
BEGIN_AP_EVTMAP(ui_ota_api_evtmap)
  ...
  CONVERT_EVENT(OTA_EVT_BURN_START,   MSG_OTA_BURNING)
