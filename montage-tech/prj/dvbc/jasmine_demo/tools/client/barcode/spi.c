...
static u32 spi_inl(u32 reg)
{
	u32 value;
	value = inl(reg);
	//OS_PRINTF("SPI_REG READ: REG %x = %x\n", reg, value);
	return value;
}
...
void spi_flash_init()
{
	...
	{//reset to set 259MHz
		...
		*((volatile u32 *)ptmp) = dtmp;
		...
		dtmp &= ~ (1 << reset_bit);
		*((volatile u32 *)ptmp) = dtmp;
		OS_PRINTF("SPI SET FREQ TO 259.2MHz !!!!!!!!!!!!!!!!!!!!!!!\n");
	}
#endif
#if 0
...
void spi_flash_read(u32 addr, u8* buf, u32 size)
{
	//u32 spiID = 0;
	spiflash_single_normal_readAny(0,addr,buf,size);
}

void spi_flash_write(u32 addr, u8* buf, u32 size)
{
	u32 spiID = 0;
