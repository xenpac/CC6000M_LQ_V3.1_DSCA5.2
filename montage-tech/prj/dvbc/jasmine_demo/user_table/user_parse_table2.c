...
void categories_mem_reset(void)
{
  ...
  un_india_ts_logic_num = 801;
  memset(&logic_info, 0, sizeof(logic_info));
  sys_status_set_status(BS_LCN_RECEIVED, FALSE);
  sys_status_save();
}

void clean_lcn_info(void)
{
  memset(&logic_info, 0, sizeof(logic_info));
}
...
BOOL add_logic_info(u16 sID, u16 logic_num)
{
  ...
  {
    logic_info.log_ch_info[i].logical_channel_id = logic_num;
  }
...
BOOL add_logic_info_aisat(lcn_channel_t *p_lcn_info)
{
  u16 i=0;
  BOOL lcn_flag = FALSE;
  ...
  if(logic_info.log_ch_num>0)
  {
    for(i=0; i<logic_info.log_ch_num; i++)
    {
      if(logic_info.log_ch_info[i].service_id == p_lcn_info->service_id)
      {
        return FALSE;
      }
    }
  ...
  logic_info.log_ch_info[logic_info.log_ch_num].service_id = p_lcn_info->service_id;
  logic_info.log_ch_info[logic_info.log_ch_num].logical_channel_id = p_lcn_info->logical_channel_id;
  logic_info.log_ch_info[logic_info.log_ch_num].bouquet_id= p_lcn_info->bouquet_id;
  logic_info.log_ch_info[logic_info.log_ch_num].show_flag= p_lcn_info->show_flag;
  logic_info.log_ch_info[logic_info.log_ch_num].sound_channel= p_lcn_info->sound_channel;
  logic_info.log_ch_info[logic_info.log_ch_num].sound_volume= p_lcn_info->sound_volume;
...
u8 find_show_flag_by_sid(u16 sID)
{
  u16 i=0;
  ...
  if(logic_info.log_ch_num>0)
  {
    for(i=0; i<logic_info.log_ch_num; i++)
    {
      if(logic_info.log_ch_info[i].service_id == sID)
      {
        return logic_info.log_ch_info[i].show_flag;
      }
    }
  }
...
u16 find_logic_number_by_sid(u16 sID)
{
  u16 i=0;
  ...
  if(logic_info.log_ch_num>0)
  {
    for(i=0; i<logic_info.log_ch_num; i++)
    {
      if(logic_info.log_ch_info[i].service_id == sID)
      {
        return logic_info.log_ch_info[i].logical_channel_id;
      }
    }
  }
...
BOOL have_logic_number(void)
{
  BOOL lcn_flag = FALSE;
  sys_status_get_status(BS_LCN_RECEIVED, &lcn_flag);
  
  if(lcn_flag == FALSE && logic_info.log_ch_num == 0)
  {
    UI_PRINTF("have not logic number\n");
    return FALSE;
  }
  else
  {
    UI_PRINTF("have logic number\n");
    return TRUE;
  }
}

u16 get_max_logic_number(void)
{
  u16 i=0;
  u16 temp = 0;
  u16 pg_id;
...
void sort_tmpgrp_by_bouquet_id(void)
{
  ...
  if(g_bouquet_grp_tmp.bouquet_count<=0)
  {
    return;
  }
  
  for(i=g_bouquet_grp_tmp.bouquet_count-1; i>0; i--)
  {
    for(j=0; j<i; j++)
    {
      if(g_bouquet_grp_tmp.bouquet_id[i]<g_bouquet_grp_tmp.bouquet_id[j])
      {
        //exchange bouquet id
        temp = g_bouquet_grp_tmp.bouquet_id[i];
        g_bouquet_grp_tmp.bouquet_id[i] = g_bouquet_grp_tmp.bouquet_id[j];
        g_bouquet_grp_tmp.bouquet_id[j] = temp;

        //exchange boq_SID_list_count
        temp = g_bouquet_grp_tmp.boq_SID_list_count[i];
        g_bouquet_grp_tmp.boq_SID_list_count[i] = g_bouquet_grp_tmp.boq_SID_list_count[j];
        g_bouquet_grp_tmp.boq_SID_list_count[j] = temp;

        //exchange boq_SID_list
        for(k=0; k<MAX_LOGIC_CHANNEL; k++)
        {
          temp = g_bouquet_grp_tmp.boq_SID_list[i][k];
          g_bouquet_grp_tmp.boq_SID_list[i][k] = g_bouquet_grp_tmp.boq_SID_list[j][k];
          g_bouquet_grp_tmp.boq_SID_list[j][k] = temp;
        }

        //exchange bouquet name
        memcpy(fav_name_unistr, g_bouquet_grp_tmp.bouquet_name[i], sizeof(fav_name_unistr));
        memcpy(g_bouquet_grp_tmp.bouquet_name[i], g_bouquet_grp_tmp.bouquet_name[j], sizeof(fav_name_unistr));
        memcpy(g_bouquet_grp_tmp.bouquet_name[j], fav_name_unistr, sizeof(fav_name_unistr));
      }
    }
  }
}
...
void parse_bat_india(handle_t handle, dvb_section_t *p_sec)
{
  ...
  if(fw_find_root_by_id(ROOT_ID_DO_SEARCH) == NULL)
  {
    if(g_bouquet_grp_tmp.bouquet_count != 0)
    {
      for(i=0; i<g_bouquet_grp_tmp.bouquet_count; i++)
      {
        if(bouquet_id == g_bouquet_grp_tmp.bouquet_id[i])
        {
    ...
    if(BouquetID_repeat)
    {
      ...
      if(hasBouquetName)
      {
        ...
        memcpy(g_bouquet_grp_tmp.bouquet_name[repeat_bouq_index], bouquet_name_unistr, MAX_BOUQUET_NAMELEN);
      }

      for(i=0; i<cur_bouquet_service_count; i++)
      {
        if(g_bouquet_grp_tmp.boq_SID_list_count[repeat_bouq_index]!=0)
        {
          for(j=0; j<g_bouquet_grp_tmp.boq_SID_list_count[repeat_bouq_index]; j++)
          {
            if(cur_bouquet_service_list_id[i]==g_bouquet_grp_tmp.boq_SID_list[repeat_bouq_index][j])
            {
              break;
            }
          }

          if(j==g_bouquet_grp_tmp.boq_SID_list_count[repeat_bouq_index])
          {
            g_bouquet_grp_tmp.boq_SID_list[repeat_bouq_index][g_bouquet_grp_tmp.boq_SID_list_count[repeat_bouq_index]] = cur_bouquet_service_list_id[i];
            g_bouquet_grp_tmp.boq_SID_list_count[repeat_bouq_index]++;
          }
        }
        else
        {
          g_bouquet_grp_tmp.boq_SID_list[repeat_bouq_index][0] = cur_bouquet_service_list_id[i];
          g_bouquet_grp_tmp.boq_SID_list_count[repeat_bouq_index]++;
        }
      }
    }
    else if((MAX_BOUQUET_GROUP - 5) <= g_bouquet_grp_tmp.bouquet_count)
    {
      UI_PRINTF("[error]==========MAX_BOUQUET_GROUP - 5 <= g_bouquet_grp.bouquet_coun ===========\n");
    }
    else
    {
           UI_PRINTF("g_bouquet_grp.bouquet_count = %d\n",g_bouquet_grp_tmp.bouquet_count);
      if(hasBouquetName)
      {
        str_asc2uni(bouquet_name, bouquet_name_unistr);
      }
      ...
      memcpy(g_bouquet_grp_tmp.bouquet_name[g_bouquet_grp_tmp.bouquet_count], bouquet_name_unistr, MAX_BOUQUET_NAMELEN);
      g_bouquet_grp_tmp.bouquet_id[g_bouquet_grp_tmp.bouquet_count] = bouquet_id;

      for(i=0; i<cur_bouquet_service_count; i++)
      {
        if(g_bouquet_grp_tmp.boq_SID_list_count[g_bouquet_grp_tmp.bouquet_count]!=0)
        {
          for(j=0; j<g_bouquet_grp_tmp.boq_SID_list_count[g_bouquet_grp_tmp.bouquet_count]; j++)
          {
            if(cur_bouquet_service_list_id[i]==g_bouquet_grp_tmp.boq_SID_list[g_bouquet_grp_tmp.bouquet_count][j])
            {
              break;
            }
          }

          if(j==g_bouquet_grp_tmp.boq_SID_list_count[g_bouquet_grp_tmp.bouquet_count])
          {
            g_bouquet_grp_tmp.boq_SID_list[g_bouquet_grp_tmp.bouquet_count][g_bouquet_grp_tmp.boq_SID_list_count[g_bouquet_grp_tmp.bouquet_count]] = cur_bouquet_service_list_id[i];
            g_bouquet_grp_tmp.boq_SID_list_count[g_bouquet_grp_tmp.bouquet_count]++;
          }
        }
        else
        {
          g_bouquet_grp_tmp.boq_SID_list[g_bouquet_grp_tmp.bouquet_count][0] = cur_bouquet_service_list_id[i];
          g_bouquet_grp_tmp.boq_SID_list_count[g_bouquet_grp_tmp.bouquet_count]++;
        }
      }
      g_bouquet_grp_tmp.bouquet_count++;
...
void create_categories(void)
{
  ...
  sys_status_set_categories_count(g_bouquet_grp.bouquet_count+FAV_LIST_COUNT);
...
void parse_nit_india(handle_t handle, dvb_section_t *p_sec)
{
  ...
  while(network_desc_len > 0)
  {
    ...
    //UI_PRINTF("TAG=[0x%x], network_desc_len=%d\n", p_buf[offset], network_desc_len);
...
#ifdef AISET_BOUQUET_SUPPORT
    else if(p_buf[offset] == HK_BOUQUET_INFO_DESCRIPTOR)
    {
      s16  bqt_data_length = 0;
      u16  bqt_data_offset = 0;
      u8 *p_bqt_des = p_buf + offset + 6;  //skip tag and len
      u8 tag_num = 0;
      u8 last_tag_num = 0;
      u8 version = 0;
      u8 bqt_info_cnt = 0;
      ...
      version = p_buf[offset+1];
      tag_num = p_buf[offset+2];
      last_tag_num = p_buf[offset+3];
      bqt_info_cnt = p_buf[offset+4];

      des_len = p_buf[offset];
      bqt_data_length = des_len - 4;
      if(bqt_info_cnt > 0)
      {
        while(bqt_data_length>0)
        {
          bqt_data_offset = 0;
          bouquet_id = MAKE_WORD2(p_bqt_des[0], p_bqt_des[1]);
          if(g_bouquet_grp.bouquet_count != 0)
          {
            for(i=0; i<g_bouquet_grp.bouquet_count; i++)
            {
          ...
          if(BouquetID_repeat)
          {
            g_bouquet_grp.bouquet_id[repeat_bouq_index] = bouquet_id;
            bqt_data_offset += 2;
            UI_PRINTF("g_bouquet_grp.bouquet_count = %d\n",g_bouquet_grp.bouquet_count);
            memset(bouquet_name, 0 ,sizeof(bouquet_name)/sizeof(u8));
            memcpy(bouquet_name, &p_bqt_des[bqt_data_offset + 1], p_bqt_des[bqt_data_offset]);
            bqt_data_offset += p_bqt_des[bqt_data_offset] + 1;
            if(strlen(bouquet_name) > 0)
            {
              str_asc2uni(bouquet_name, bouquet_name_unistr);
            }
            else
            ...
            memcpy(g_bouquet_grp.bouquet_name[repeat_bouq_index], bouquet_name_unistr, MAX_BOUQUET_NAMELEN);
            p_bqt_des += bqt_data_offset;
            bqt_data_length -= bqt_data_offset;
          }
          else if((MAX_BOUQUET_GROUP - 5) <= g_bouquet_grp.bouquet_count)
          {
            UI_PRINTF("[error]==========MAX_BOUQUET_GROUP - 5 <= g_bouquet_grp.bouquet_coun ===========\n");
      ...
      network_desc_len -=  des_len;
      offset   +=  des_len;
      offset ++;
      network_desc_len -- ;
    }
    else if(p_buf[offset] == HK_BOUQUET_LCN_DESCRIPTOR)
    {
      lcn_channel_t log_ch_info = {0};
      u8  lcn_data_length = 0;
      u8 *p_log_des = p_buf + offset + 6;  //skip tag and len
      u8 tag_num = 0;
      u8 last_tag_num = 0;
      u8 version = 0;
      u8 lcn_info_cnt = 0;
      ...
      tag_num = p_buf[offset+2];
      last_tag_num = p_buf[offset+3];
      lcn_info_cnt = p_buf[offset+4];
      
      lcn_data_length = des_len - 4;
      
      network_desc_len -=  des_len;
      offset   +=  des_len;
      if(lcn_info_cnt > 0)
      {
        while(lcn_data_length>0)
        {
          log_ch_info.bouquet_id = MAKE_WORD2(p_log_des[0], p_log_des[1]);
          log_ch_info.service_id = MAKE_WORD2(p_log_des[2], p_log_des[3]);
          log_ch_info.logical_channel_id = MAKE_WORD2(p_log_des[4], p_log_des[5]);
          log_ch_info.show_flag = p_log_des[6];
          log_ch_info.sound_channel = p_log_des[7];
          log_ch_info.sound_volume = p_log_des[8];
          add_logic_info_aisat(&log_ch_info);

          //UI_PRINTF("[parsebat]s_id = %d, logic =%d\n", service_id, logical_channel_id);
          p_log_des += 9;
          if(lcn_data_length < 9) //for check error
          {
            lcn_data_length = 0;
          }
          else
  ...
  while(ts_loop_len > 0)
  {
    ...
    while(desc_len > 0)
    {
      ...
      { 
        ...
        {
          //mtos_printk("svc_list_des_length[%d]\n", svc_list_des_length);
#ifdef QUICK_SEARCH_SUPPORT
          p_nit->tp_svc_list[tp_index].svc_id[svc_cnt] 
        ...
        p_nit->tp_svc_list[tp_index].total_svc_num = svc_cnt;
        tp_index++;
          break;
  ...
  {
    u16 i = 0, j = 0, k = 0;
    if(logic_info.log_ch_num >= 0 && g_bouquet_grp.bouquet_count > 0)
    {
      for(i=0; i<logic_info.log_ch_num; i++)
      {
        for(j=0; j<g_bouquet_grp.bouquet_count; j++)
        {
          if(logic_info.log_ch_info[i].bouquet_id == g_bouquet_grp.bouquet_id[j])
          {
            if(g_bouquet_grp.boq_SID_list_count[j]!=0)
            {
              for(k=0; k<g_bouquet_grp.boq_SID_list_count[j]; k++)
              {
                if(logic_info.log_ch_info[i].service_id==g_bouquet_grp.boq_SID_list[j][k])
                {
                  break;
                }
              }

              if(k==g_bouquet_grp.boq_SID_list_count[j])
              {
                g_bouquet_grp.boq_SID_list[j][k] = logic_info.log_ch_info[i].service_id;
                g_bouquet_grp.boq_SID_list_count[j]++;
              }
            }
            else
            {
              g_bouquet_grp.boq_SID_list[j][0] = logic_info.log_ch_info[i].service_id;
              g_bouquet_grp.boq_SID_list_count[j]++;
            }
          }
        }
...
if(network_id != 0xFFFF)
{
  ...
  p_sec->filter_code[3] = 0x01;
  p_sec->filter_mask[3] = 0x01;  
