...
static control_t *create_network_config_menu()
{
   ...
   p_menu = ui_comm_root_create_netmenu(ROOT_ID_NETWORK_CONFIG_LAN,0, IM_INDEX_NETWORK_BANNER, IDS_LAN);
  ...
  for(i = 0;i < IP_CTRL_LAN_CNT;i++)
  {
     switch(i)
      {
        ...
        text_set_font_style(p_ctrl[i], FSI_WHITE, FSI_WHITE, FSI_WHITE);
    		...
    		strcpy((char*)connect_type, "Link Type:                      LAN");
    		text_set_content_by_ascstr(p_ctrl[i], connect_type);		 
        break;
        case 1:
          p_ctrl[i] = ui_comm_select_create(p_menu, (u8)(IDC_CONNECT_TYPE + i), NETWORK_LAN_ITEM_X, y, NETWORK_LAN_ITEM_LW, NETWORK_LAN_ITEM_RW);
          ...
          if(g_net_config.link_type == LINK_TYPE_WIFI)
          {
            ...
            {
               ui_comm_select_set_content(p_ctrl[i], (u8)j, network_str_wifi[j]);
            }
     ...
     //       ui_comm_select_create_droplist(p_ctrl[i], CONFIG_MODE_TYPE_CNT - 1);
          ...
          else
          {
            ...
            {
               ui_comm_select_set_content(p_ctrl[i], (u8)j, network_str[j]);
            }
          ...
          ui_comm_select_create_droplist(p_ctrl[i], CONFIG_MODE_TYPE_CNT);
			break;		
        case 2:
          p_ctrl[i] = ui_comm_ipedit_create(p_menu, (u8)(IDC_CONNECT_TYPE + i), NETWORK_LAN_ITEM_X, y, NETWORK_LAN_ITEM_LW, NETWORK_LAN_ITEM_RW);
        ...
        case 6:
          ...
          p_ctrl[i] = ui_comm_static_create(p_menu, (u8)(IDC_CONNECT_TYPE + i), NETWORK_LAN_ITEM_X, y, NETWORK_LAN_ITEM_LW, NETWORK_LAN_ITEM_RW);
...
void paint_connect_status(BOOL is_conn, BOOL is_paint)
{
  ...
  p_conn_status = ui_comm_root_get_ctrl(ROOT_ID_NETWORK_CONFIG_LAN, IDC_CONNECT_STATUS);
...
static void create_connect_status_ctrl(control_t *p_menu)
{
  ...
  OS_PRINTF("################print connect status, %d,%d,%d,%d############\n", eth_connt_stats.is_eth_conn,eth_connt_stats.is_eth_insert,eth_connt_stats.is_wifi_conn,eth_connt_stats.is_wifi_insert);
  if(eth_connt_stats.is_eth_conn)
  {
...
static void static_ip_to_dhcp()
{
  ...
  for(i = 0;i < 4;i++)
  {
    ...
    {
      case 0:
        p_ctrl[i] = ui_comm_root_get_ctrl(ROOT_ID_NETWORK_CONFIG_LAN, IDC_IP_ADDRESS);
        memcpy(&temp_address, &(ss_ip.sys_ipaddress), sizeof(ip_addr_t));
        ui_comm_ipedit_set_address(p_ctrl[i], &temp_address);
       break;
      case 1:
        p_ctrl[i] = ui_comm_root_get_ctrl(ROOT_ID_NETWORK_CONFIG_LAN, IDC_NETMASK);
        memcpy(&temp_address, &(ss_ip.sys_netmask), sizeof(ip_addr_t));
        ui_comm_ipedit_set_address(p_ctrl[i], &temp_address);
       break;
      case 2:
        p_ctrl[i] = ui_comm_root_get_ctrl(ROOT_ID_NETWORK_CONFIG_LAN, IDC_GATEWAY);
        memcpy(&temp_address, &(ss_ip.sys_gateway), sizeof(ip_addr_t));
        ui_comm_ipedit_set_address(p_ctrl[i], &temp_address);
       break;
      case 3:
        p_ctrl[i] = ui_comm_root_get_ctrl(ROOT_ID_NETWORK_CONFIG_LAN,IDC_DNS_SERVER);
        memcpy(&temp_address, &(ss_ip.sys_dnsserver), sizeof(ip_addr_t));
        ui_comm_ipedit_set_address(p_ctrl[i], &temp_address);
       break;   
     default:
...
static void update_ipbox_attr(BOOL is_enable)
{
  ...
  for(i = 0;i < 4;i++)
  {
    p_ctrl[i] = ui_comm_root_get_ctrl(ROOT_ID_NETWORK_CONFIG_LAN, (u8)(IDC_IP_ADDRESS + i));
  ...
  u32 para1, u32 para2)
{
  ...
  p_ctrl = ui_comm_root_get_ctrl(ROOT_ID_NETWORK_CONFIG_LAN, IDC_CONNECT_NETWORK);
  switch(focus)
  {
    case 0:
...
BOOL check_is_connect_ok(ethernet_device_t *p_eth_dev)
{
  ...
  #ifndef WIN32
  get_net_device_addr_info(p_eth_dev,(char*)ipaddr,(char*)netmask,(char*)gw,(char*)primarydns,(char*)alternatedns);
  #endif
